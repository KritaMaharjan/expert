              ...
              <div class="box box-solid">
                <div class="box-header">
                  ...
                  <div class="icon-block">
                    <a href="javascript:;" title="Edit" data-original-title="Edit" class="btn btn-box-tool" data-toggle="modal" data-url="{{tenant()->url('supplier/'.$supplier['id'].'/edit')}}" data-target="#fb-modal"><i class="fa fa-edit"></i></a>
                   
                    <a href="javascript:;" class="btn-delete-supplier_each" title="Delete"><i class="fa fa-trash-o"></i></a>
                  </div>
                </div><!-- /.box-header -->
                <div class="box-body">
                  <div class="supplier-info">
                    <p><strong>{{$supplier['name']}}</strong></p>
                    <p><strong>{{$supplier->dob}}</strong></p>
                    <p class="supplier-address">
                      <strong>{{$supplier->street_name}}  {{$supplier->street_number}}</strong><br />
                      <strong>{{$supplier->postcode}} {{$supplier->town}}</strong><br />
                      <strong></strong>
                    </p>
                    <p><strong>{{$supplier->telephone}}</strong> <small>(Telephone)</small><br />
                       <strong>{{$supplier->mobile}}</strong> <small>(Mobile)</small>
                    </p>
                   <a href="#" data-toggle="modal" data-target="#compose-modal">{{$supplier->email}}</a>

                    <div class="form-group radio-box">
                      <label>
                        <input type="radio" name="r1" value="1" class="minimal minimal_status" <?php if($supplier->status == 1) echo 'checked=checked '; ?> /> Active
                      </label>
                      <label>
                        <input type="radio" name="r1"  value="0" class="minimal minimal_status" <?php if($supplier->status == 0) echo 'checked=checked '; ?> /> Inactive
                      </label>
             ...
             <div class="box box-solid">
                <div class="box-header">
                  ...
                  <div class="btn-top-ryt">
                    <a href="{{tenant()->url('invoice/offer/add?id=')}}{{$supplier->id}}" class="btn btn-primary btn-block btn-flat">New Offer
                    </a>
                    <a href="{{tenant()->url('invoice/bill/add?id=')}}{{$supplier->id}}" class="btn btn-primary btn-block btn-flat">New Bill
                    </a>
                  </div>
                </div>
                <div class="box-body">
                              ...
                              <div class="col-sm-12 search-form pad-5">
                                <form class="text-right" action="#">
                                  <div class="input-group">
                                     <input type="hidden" id="user_id" class="form-control input-sm " value="{{$supplier->id}}">
                    ...
                    <div class="table-blk">
                        ...
                        <div class="input-group">
                            <span class="input-group-addon">TO:</span>
                             {!! Form::text('email_to',$supplier->email, ['id'=>'email_to','class'=>'form-control', 'placeholder'=>'Email To', 'autocomplete'=>'off']) !!}
                        </div>
                    </div>
                    <div class="form-group disply-inline">
                        <div class="input-group">
...
<script type="text/javascript">
    $(function(){
        ...
        $(".textarea").wysihtml5();
          ...
          <script type="text/javascript">
        console.log(appUrl+'supplier/changeStatus');
          $( ".minimal_status" ).click(function() {
            
            var cus_id = "{{$supplier['id']}}";
            var token = "{{ csrf_token()}}";
            var url_to = appUrl+'supplier/changeStatus';
            var status = $(this).val();
        ...
        $(document).on('click', '.btn-delete-supplier_each', function (e) {
        e.preventDefault();
        var $this = $(this);
        ...
        if (cus_id != '' && doing == false) {
            ...
            $.ajax({
                url: appUrl + 'supplier/' + cus_id + '/delete',
                type: 'GET',
                dataType: 'json',
            })
                .done(function (response) {
